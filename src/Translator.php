		...
		if (!isset($this->data[$locale])) {
			...
			if (!file_exists($localeCache) || ($this->debugMode && $dataLoader->isLocaleUpdated($locale, $localeCache))) {
				...
				file_put_contents(
					$localeCache . '.tmp',
					sprintf(
						'<?php declare(strict_types=1); class TranslatorData%s extends Forrest79\SimpleTranslator\TranslatorData {protected function getPluralIndex(int $count): int {%sthrow new Forrest79\SimpleTranslator\TranslatorException(\'No definition for count \' . $count);}}; return new TranslatorData%s(\'%s\', [%s]);',
