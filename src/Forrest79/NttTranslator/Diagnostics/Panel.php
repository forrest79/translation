<?php

namespace Forrest79\NttTranslator\Diagnostics;

use Tracy;
use Tracy\Helpers;


class Panel implements Tracy\IBarPanel
{
	/** @var array */
	private $untranslated = [];

	/** @var int */
	private $untranslatedCount;

	/** @var array */
	private $localeFiles = [];


	/**
	 * Renders HTML code for custom tab.
	 * @return string
	 */
	public function getTab()
	{
		return '<span title="Translation"><img width="16px" height="16px" src="data:image/png;base64," />'
			. implode(', ', array_keys($this->localeFiles)) . ($this->untranslated ? ' <strong>(' . $this->getUntranslatedCount() . ' errors)</strong>' : '')
			. '</span>';
	}


	/**
	 * Renders HTML code for custom panel.
	 * @return string
	 */
	public function getPanel()
	{
		$panel = [];
		if ($this->untranslated) {
			foreach ($this->untranslated as $locale => $untranslated) {
				$panel[] = $this->renderUntranslated($locale, $untranslated);
			}
		}

		if ($this->localeFiles) {
			foreach ($this->localeFiles as $locale => $localeFile) {
				if ($panel) {
					$panel[] = '<br><br>';
				}
				$panel[] = '<h2>Loaded locale files</h2>';
				$panel[] = $this->renderLocaleFiles();
			}
		}

		return $panel ?
			'<h1>Missing translations: ' . $this->getUntranslatedCount() . '</h1>' .
			'<div class="nette-inner tracy-inner translator-panel" style="min-width:500px">' . implode($panel) . '</div>' .
			'<style>
				#tracy-debug .translator-panel h2 {font-size: 23px;}
			</style>' : '';
	}


	private function getUntranslatedCount()
	{
		if ($this->untranslatedCount === NULL) {
			$count = 0;
			foreach ($this->untranslated as $untranslated) {
				$count += count(array_unique($untranslated, SORT_REGULAR));
			}
			$this->untranslatedCount = $count;
		}
		return $this->untranslatedCount;
	}


	private function renderUntranslated($locale, $untranslated)
	{
		$s = '';

		foreach ($unique = array_unique($untranslated, SORT_REGULAR) as $message) {
			$s .= '<tr><td>' . htmlSpecialChars($message) . '</td></tr>';
		}

		return '<table style="width:100%"><tr><th>Untranslated messages (' . $locale . ')</th></tr>' . $s . '</table>';
	}


	private function renderLocaleFiles()
	{
		$s = '';

		foreach ($this->localeFiles as $locale => $localeFile) {
			$s .= '<tr>';
			$s .= '<td>' . htmlSpecialChars($locale) . '</td>';
			$s .= '<td>' . Helpers::editorLink($localeFile, 1) . '</td>';
			$s .= '</tr>';
		}

		return '<table style="width:100%"><tr><th>Locale</th><th>Filename</th></tr>' . $s . '</table>';
	}


	/**
	 * @param string $locale
	 * @param string $message
	 */
	public function addUntranslated($locale, $message)
	{
		$this->untranslated[$locale][] = $message;
	}


	/**
	 * @param string $locale
	 * @param string $localeFile
	 */
	public function addLocaleFile($locale, $localeFile)
	{
		$this->localeFiles[$locale] = $localeFile;
	}


	public static function register()
	{
		$panel = new static;

		Tracy\Debugger::getBar()->addPanel($panel, 'translator');

		return $panel;
	}

}
